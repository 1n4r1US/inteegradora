...
switch ($method) {
    ...
    case 'POST':
        // Crear consultorio (solo psicólogos y admin)
        if ($_SESSION['rol'] !== 'psicologo' && $_SESSION['rol'] !== 'admin') {
            http_response_code(403);
            echo json_encode(['success' => false, 'error' => 'Solo psicólogos pueden crear consultorios']);
            break;
        }
    ...
    case 'PUT':
        // Actualizar consultorio (solo psicólogos y admin)
        if ($_SESSION['rol'] !== 'psicologo' && $_SESSION['rol'] !== 'admin') {
            http_response_code(403);
            echo json_encode(['success' => false, 'error' => 'Solo psicólogos pueden modificar consultorios']);
            break;
        }
    ...
    case 'DELETE':
        // Eliminar consultorio (solo psicólogos y admin)
        if ($_SESSION['rol'] !== 'psicologo' && $_SESSION['rol'] !== 'admin') {
            http_response_code(403);
            echo json_encode(['success' => false, 'error' => 'Solo psicólogos pueden eliminar consultorios']);
            break;
        }

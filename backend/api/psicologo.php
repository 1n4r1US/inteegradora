...
switch ($method) {
    ...
    case 'PUT':
        ...
        // Validar que el psicólogo solo pueda actualizar su propia información
        ...
        if ($stmt->execute()) {
            // Si se envió teléfono, actualizar tabla usuarios
            if ($telefono !== null) {
                $sql_user = "UPDATE usuarios SET telefono=? WHERE user_id=?";
                $stmt_user = $conn->prepare($sql_user);
                $stmt_user->bind_param('si', $telefono, $user_id);
                $stmt_user->execute();
                $stmt_user->close();
            }
